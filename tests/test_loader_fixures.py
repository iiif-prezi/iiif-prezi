...
class TestAll(unittest.TestCase):
    ...
    def test01_fixtures(self):
        ...
        for manifest in ncoll.manifests:
            mfid = manifest.id
            fn = mfid.replace(
                'http://iiif.io/api/presentation/', 'tests/testdata/')
            fh = open(fn)
            data = fh.read()
            fh.close()
            diff = ''
    ...
    def test02_errors(self):
        ...
        # was ConfigurationError
        self.assertRaises(RequirementError, error, 23)
        ...
        # was ConfigurationError
        self.assertRaises(RequirementError, error, 42)
